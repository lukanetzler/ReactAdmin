...
export default function TwilightSpaceView({ onBack, user, initialCard = null, onInitialCardConsumed }) {
  ...
  const saveToJournal = useCallback(async (title, cardTitle) => {
    ...
    const reflection = `✨ The Night Sky\n\n${title}\n${cardTitle}`;
    ...
    return (
      <div className="fixed inset-0 flex flex-col items-center justify-center font-sans" style={{ background: BG }}>
        <div className="w-16 h-16 rounded-full flex items-center justify-center mb-6"
          ...
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={AMBER} strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/>
          </svg>
        </div>
        <p className="text-[9px] font-bold tracking-[0.4em] mb-3" style={{ color: 'rgba(212,163,115,0.5)' }}>THE NIGHT SKY</p>
        ...
        <p className="text-sm text-center leading-relaxed mb-12 max-w-[260px]"
          style={{ color: 'rgba(253,249,243,0.4)' }}>
        ...
        <button onClick={returnFromDone}
          ...
          style={{ backgroundColor: 'rgba(212,163,115,0.15)', color: AMBER, border: '1px solid rgba(212,163,115,0.2)' }}>
          RETURN
        </button>
      </div>
    );
  ...
  if (activeTrack) {
    return (
      <div className="fixed inset-0 flex flex-col font-sans"
        style={{ background: 'linear-gradient(180deg, #130e08 0%, #1e1610 40%, #2a1e10 100%)' }}>
        ...
        <div className="flex-shrink-0 flex items-center justify-between px-6 pt-14 pb-4">
          <button
            ...
            <ChevronRight className="rotate-180" size={18} color={AMBER} />
          </button>
          <span className="text-[10px] tracking-[0.3em] font-bold" style={{ color: 'rgba(212,163,115,0.4)' }}>THE NIGHT SKY</span>
          <div className="w-10" />
        </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          <div className="relative flex flex-col items-center">
            <div className="absolute rounded-full pointer-events-none" style={{
              ...
              filter: 'blur(10px)',
              opacity: isPlaying ? 0.14 : 0,
              transition: 'opacity 2s ease',
            }} />
            <svg viewBox="0 0 100 120" style={{ width: 100, height: 126, filter: 'drop-shadow(0 0 10px rgba(212,163,115,0.2))', position: 'relative', zIndex: 1 }}>
              <g fill="#1e1610" opacity="1">
              ...
              <path fill="#C87941" d="M50,110 C32,110 28,92 50,48 C72,92 68,110 50,110 Z" style={{
              ...
              <path fill="#D4A373" d="M50,108 C40,108 34,98 50,65 C66,98 60,108 50,108 Z" style={{
                transformOrigin: 'bottom center', filter: 'blur(0.5px)',
                opacity: isPlaying ? 1 : 0, transition: 'opacity 1.5s ease',
                animation: isPlaying ? 'flame-flicker 2.6s ease-in-out infinite' : 'none',
              }} />
              <path fill="#F0C898" d="M50,106 C44,106 40,102 50,80 C60,102 56,106 50,106 Z" style={{
                transformOrigin: 'bottom center', filter: 'blur(0.5px)',
                opacity: isPlaying ? 1 : 0, transition: 'opacity 1.5s ease',
          ...
          <div className="text-center px-8 mt-4">
            <p className="text-[10px] tracking-[0.3em] font-bold mb-1" style={{ color: 'rgba(212,163,115,0.4)' }}>
              {activeTrack.card.title}
            </p>
            <h2 className="text-2xl font-serif leading-snug" style={{ color: CREAM }}>
              {activeTrack.track.title || activeTrack.card.title}
            </h2>
            <div className="mt-3 mx-auto w-8 h-px" style={{ backgroundColor: 'rgba(212,163,115,0.3)' }} />
          </div>
        </div>

        <div className="flex-shrink-0 px-6 pb-12 space-y-5">
          <div>
            <div ref={progressRef} className="relative w-full h-7 flex items-center cursor-pointer"
              onClick={e => seekTo(e.clientX)}
              onTouchStart={e => seekTo(e.touches[0].clientX)}
              onTouchMove={e => { e.preventDefault(); seekTo(e.touches[0].clientX); }}>
              <div className="absolute w-full h-0.5 rounded-full overflow-hidden" style={{ backgroundColor: 'rgba(212,163,115,0.2)' }}>
                <div className="h-full rounded-full" style={{ width: `${progress}%`, backgroundColor: AMBER }} />
              </div>
              <div className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-3 h-3 rounded-full"
                style={{ left: `${progress}%`, backgroundColor: '#F0C898', boxShadow: '0 0 8px rgba(240,200,152,0.5)' }} />
            </div>
            <div className="flex justify-between text-[10px] font-mono -mt-1" style={{ color: 'rgba(212,163,115,0.4)' }}>
          ...
          <div className="flex items-center justify-between px-4">
            ...
            <button onClick={togglePlay}
              ...
              style={{
                ...
                boxShadow: '0 0 30px rgba(212,163,115,0.15)',
                color: AMBER,
              }}>
              {isPlaying
          ...
          <button onClick={onCompleteSession} disabled={saving}
            ...
            style={{ backgroundColor: 'rgba(212,163,115,0.08)', border: '1px solid rgba(212,163,115,0.15)', color: 'rgba(212,163,115,0.55)' }}>
            {saving ? 'Saving…' : 'Done'}
          </button>
        </div>
      </div>
      ...
      <div className="fixed inset-0 flex flex-col font-sans" style={{ background: BG }}>
        <div className="flex-shrink-0 flex items-center justify-between px-6 pt-14 pb-4">
          <button onClick={() => setActiveArticle(null)}
            className="w-10 h-10 rounded-full flex items-center justify-center"
            style={{ backgroundColor: 'rgba(212,163,115,0.12)' }}>
            <ArrowLeft size={18} color={AMBER} strokeWidth={1.5} />
          </button>
          <span className="text-[10px] tracking-[0.3em] font-bold" style={{ color: 'rgba(212,163,115,0.4)' }}>THE NIGHT SKY</span>
          <div className="w-10" />
        </div>
        <div className="flex-1 overflow-y-auto px-6 pb-6">
          <h1 className="text-3xl font-serif leading-tight mb-2 mt-2" style={{ color: CREAM }}>
            {activeArticle.card.title}
          </h1>
          {activeArticle.card.label && (
            <p className="text-[9px] font-bold tracking-[0.35em] mb-6" style={{ color: 'rgba(212,163,115,0.6)' }}>
              {activeArticle.card.label}
            </p>
          )}
          <div className="h-px mb-6" style={{ backgroundColor: 'rgba(212,163,115,0.12)' }} />
          <p className="text-sm leading-relaxed whitespace-pre-wrap" style={{ color: 'rgba(253,249,243,0.8)' }}>
            ...
            <motion.div
              ...
              {selectedCard.imageUrl && (
                <div className="relative h-40 mx-4 mt-3 rounded-[20px] overflow-hidden">
                  ...
                  <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(26,18,10,0.7) 0%, transparent 60%)' }} />
                </div>
              )}
              ...
              <div className="px-6 pt-4 flex items-start justify-between">
                <div>
                  {selectedCard.label && (
                    <p className="text-[9px] font-bold tracking-widest mb-1" style={{ color: 'rgba(212,163,115,0.55)' }}>
                      {selectedCard.label}
                    </p>
                  )}
                  <p className="text-xl font-serif" style={{ color: CREAM }}>{selectedCard.title}</p>
                  {selectedCard.duration && (
                    <p className="text-[10px] mt-0.5" style={{ color: 'rgba(212,163,115,0.4)' }}>{selectedCard.duration}</p>
                  )}
                </div>
              ...
              {selectedCard.description && (
                <p className="px-6 mt-3 text-sm leading-relaxed" style={{ color: 'rgba(253,249,243,0.45)' }}>
                  {selectedCard.description}
                </p>
              )}
                  ...
                  <div className="flex flex-col items-center gap-3">
                    <button
                      ...
                      style={{
                        backgroundColor: 'rgba(212,163,115,0.15)',
                        border: '1px solid rgba(212,163,115,0.3)',
                        boxShadow: '0 0 30px rgba(212,163,115,0.12)',
                        color: AMBER,
                      }}>
                      <Play fill="currentColor" size={28} className="ml-1" />
                    </button>
                    {firstTrack?.duration && (
                      <p className="text-[10px] font-bold tracking-widest" style={{ color: 'rgba(212,163,115,0.4)' }}>
                        {firstTrack.duration}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    {(selectedCard.tracks || []).map((track, i) => (
                      <button key={i}
                        ...
                        style={{ backgroundColor: 'rgba(212,163,115,0.07)', border: '1px solid rgba(212,163,115,0.1)' }}>
                        <div className="w-9 h-9 rounded-[10px] flex items-center justify-center flex-shrink-0"
                          style={{ backgroundColor: 'rgba(212,163,115,0.12)' }}>
                          <Play size={12} fill={AMBER} color={AMBER} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-serif truncate" style={{ color: CREAM }}>{track.title}</p>
                          {track.duration && <p className="text-[9px] mt-0.5" style={{ color: 'rgba(212,163,115,0.4)' }}>{track.duration}</p>}
                        </div>
                        <ChevronRight size={14} color="rgba(212,163,115,0.35)" strokeWidth={1.5} />
                      </button>
                    ))}
                  </div>
                )}
